...
const YourBookings = () => {
    ...
    const handleDelete = (id: any) => {
        const proced = window.confirm("Are Your Sure Delete This Events");
        if (proced) {
            ...
            fetch(url, {
                ...
                .then((res) => res.json())
                .then((data) => {
                    const reamingData = booking.filter((events: any) => events._id !== id);
                    setBooking(reamingData);
                    toast.success("Your Events Succesfully deleted");
                });
        }
    };
    return (
                                ...
                                booking.map((item: any) => (<div className="mt-6 md:mt-0 flex justify-start flex-col md:flex-row  items-start md:items-center space-y-4  md:space-x-6 xl:space-x-8 w-full mb-5">
                                    ...
                                    <div className="  flex justify-between items-start w-full flex-col md:flex-row space-y-4 md:space-y-0  ">
                                        ...
                                        <div className="flex justify-between space-x-8 items-start w-full">
                                            ...
                                            <p onClick={() => handleDelete(item._id)} className="text-2xl text-primary cursor-pointer"> <RiDeleteBin6Line /></p>
                                        </div>
                                    </div>
                                </div>))
                            }
