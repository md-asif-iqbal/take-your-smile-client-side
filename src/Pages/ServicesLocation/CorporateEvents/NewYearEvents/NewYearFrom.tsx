...
import UseNewYear from "../../../../hooks/Decration/Corporate/UseNewYear";
const NewYearFrom = () => {
  ...
  const { newy } = useParams();
  const [newYear]: any = UseNewYear(newy);
  console.log(packag);
  const { id } = useParams();
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    const booking = {
      ...
      email: user?.email,
      decrImg1: newYear.decImg,
      decrImg2: newYear.decImg1,
      decrImg3: newYear.decImg2,
