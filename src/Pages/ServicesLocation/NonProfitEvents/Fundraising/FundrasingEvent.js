...
const FundrasingEvent = () => {
  const [packages] = UsePack();
  const { id } = useParams();
  const [nonProfit] = UseNonProfit(id);
  const navigate = useNavigate();
  const handleClick = (item) => {
    if (item) {
      navigate(`/fundrasLoc/${item}`);
    }
    console.log(item);
  };
  return (
    <div>
      <h1 className="text-3xl text-center font-mono mt-10 font-bold">
        You Are Select
        <span className="text-primary font-mono"> {nonProfit.name2}</span>
      </h1>
...
export default FundrasingEvent;
