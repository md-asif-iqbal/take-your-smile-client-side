...
const SocialLogin = () => {
  ...
  const from = location.state?.from?.pathname || '/';    const [signInWithGoogle, Guser, Gloading, Gerror] = useSignInWithGoogle(auth);
    ...
    return (
        ...
        <button className="social-icon hover:text-primary text-secondary border-primary border-2" onClick={() => signInWithFacebook()}>
          <BsFacebook />
        </button>
      
        <button className="social-icon hover:text-primary text-secondary border-primary border-2" onClick={() => signInWithGoogle()}>
          <BsGoogle />
        </button>
